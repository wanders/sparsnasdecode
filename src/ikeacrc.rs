/// This implements crc16 with parameters used by the sparsnas transmitter
/// (polynomial 0x8005 and init value 0xffff)
...
pub fn crc(data: &[u8]) -> u16 {
    let mut crc: u16 = 0xeaa8;
    for b in data {
        crc = ((crc << 8) | (*b as u16)) ^ TABLE[((crc >> 8) & 0xFF) as usize];
    }
    crc = (crc << 8) ^ TABLE[((crc >> 8) & 0xFF) as usize];
    (crc << 8) ^ TABLE[((crc >> 8) & 0xFF) as usize]
}
...
mod tests {
    ...
    fn realworld() {
        let testdata = [
            0x11, 0xe0, 0x54, 0x07, 0x0e, 0xa2, 0x1d, 0x28, 0xa7, 0x8d, 0x76, 0x14, 0x1a, 0x47,
            0x8b, 0x95, 0x0a, 0x14,
        ];

        let crc = crate::ikeacrc::crc(&testdata);
        assert_eq!(crc, 0xaaed);
    }
    ...
    fn zeros() {
        ...
        let crc = crate::ikeacrc::crc(&testdata);
        assert_eq!(crc, 0xfd02);
    }

    #[test]
    fn rustcrc() {
        let testdata = b"123456789";
        let crc = crate::ikeacrc::crc(testdata);
